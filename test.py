...
# Enlever les en-têtes et les pieds de page PEM
def remove_pem_headers(pem_str):
    lines = pem_str.decode('utf-8').splitlines()
...
def generateur_key():
    ...
    public_key_base64=remove_pem_headers(public_key_pem )
    private_key_base64=remove_pem_headers(private_key_pem )
    RSA_result_gen_pb.config(text=public_key_base64) 
    RSA_result_gen_pv.config(text=private_key_base64) 
...
def load_rsa_private_key(base64_key):
    try:
        # Ajoutez des en-têtes et pieds de page si nécessaires
        if not base64_key.startswith("-----BEGIN PRIVATE KEY-----"):
            base64_key = f"-----BEGIN PRIVATE KEY-----\n{base64_key}\n-----END PRIVATE KEY-----"
        ...
        messagebox.showerror("Invalid Key", f"Failed to load private key: {str(e)}")
        return None

def load_rsa_public_key(base64_key):
    try:
        # Ajoutez des en-têtes et pieds de page si nécessaires
        if not base64_key.startswith("-----BEGIN PUBLIC KEY-----"):
            base64_key = f"-----BEGIN PUBLIC KEY-----\n{base64_key}\n-----END PUBLIC KEY-----"
...
def RSA_run():
    ...
    public_key = load_rsa_public_key(public_key_base64)
...
def RSA_run_dec():
    ...
    private_key = load_rsa_private_key(private_key_base64)
    if private_key is None:
        return None
    
    encrypted_text = RSA_dec_entry.get().strip()
